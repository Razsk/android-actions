...
4. As a user, I want to see open due routines and tasks in a prominent 'Due Objectives' HUD section on Mission Control so that I immediately know what needs action.
5. As a user, I want an inviting solid Action Blue button in the header and a Floating Action Button (FAB) at the bottom-right so that creating a task is always accessible.
6. As a user, I want to switch to a dedicated 'Stats & Analytics' bottom navigation tab so that I can view my completion performance metrics.
7. As a user, I want to view an Overall Reliability % gauge and a 7-day Cyber Cyan Consistency Heatmap grid so that I can visualize my weekly task completion volume.
8. As a user, I want to view an Hourly Peak Productivity distribution chart so that I know what time of day I complete tasks most efficiently.
9. As a user, I want to create new custom tags and lists inline within the Task Creation modal so that I don't have to exit my task creation flow.
10. As a user, I want a 'REUSABLE TASK' switch toggle in the Task Creation modal so that I can choose whether to keep a task definition in my library for future re-selection.
11. As a user, I want to assign tags and lists to tasks so that I can organize objectives by context (Work, Health, Home).
12. As a user, I want to define recurring routines with default frequency periods so that due tasks auto-appear on my Mission Control view.
13. As a user, I want to postpone a due routine with a single tap using a pre-defined deferral period so that it reschedules without extra prompts unless a custom period is required.
14. As a user, I want Gemma to analyze my routine postponement history so that I receive non-intrusive cards suggesting baseline schedule shifts.
15. As a user, I want to accept or dismiss Gemma's schedule shift suggestions with a single tap.
16. As a user, I want to launch single-day challenge modules (e.g. 30-Min Apartment Reset) so that I can complete grouped tasks within a time budget.
17. As a user, I want to see an uncluttered Focus HUD screen during an active challenge so that I only focus on the active task and remaining timer.
18. As a user, I want a Ghost Pace Bar during challenges so that I can compare my current split time against my personal best attempt.
19. As a user, I want to see a Challenge Summary screen upon completing all tasks so that I can view total time elapsed, personal best split deltas, and share progress.
20. As a user, I want to send challenge start, finish, and timeout updates to a buddy via standard SMS or messaging apps without requiring them to install the app.
